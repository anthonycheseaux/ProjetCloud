 ...
 */
public class EndpointsAsyncTaskDelete extends AsyncTask<Pair<Context, List<List>>, Void, String> {
        ...
        try {
            if (artistList != null) {
                for(Artist artist : artistList) {
                    artistApi.remove(artist.getId()).execute();
                }
            }

            if (artworkList != null) {
                for(Artwork artwork : artworkList) {
                    artworkApi.remove(artwork.getId()).execute();
                }
            }

            if (roomList != null) {
                for(Room room : roomList) {
                    roomApi.remove(room.getId()).execute();
                }
            }
